...
> This explainer moved from the repository root so [`README.md`](../README.md) can document the `webmcp-agents` project. The normative source remains [`index.bs`](../index.bs).

WebMCP is a proposed web standard that helps developers build and expose structured tools for AI agents. It provides JavaScript and annotations for HTML form elements so agents know how to interact with page features and support a user's experience. WebMCP is a progressive enhancement: the page remains usable without it, while tools can improve the performance and reliability of agent actuation.
...
This repository contains the WebMCP specification, explainers, proposals, and a repo-local skill surface for working with WebMCP. [`index.bs`](../index.bs) is the sole normative source. [`ARCHITECTURE.md`](../ARCHITECTURE.md) explains the repository and data flow; [`skills/catalog.md`](../skills/catalog.md) indexes the eleven focused skills.

The skills describe how to author, inspect, secure, evaluate, and maintain page tools. [`webmcp-design-md`](../skills/webmcp-design-md/SKILL.md) additionally describes how a page-owned WebMCP tool can support a DESIGN.md pipeline. These documents do not add browser APIs or provide an agent runtime.
...
**WebMCP** introduces a client-side alternative. It allows web developers to define tools directly in the browser page's script. This enables cooperative interplay between a user, a web page, and an agent with shared context. Tools execute on the webpage visibly, so users can trust that tasks are completed as expected while the page keeps its brand and human-centered design choices.
...
await document.modelContext.registerTool({
  ...
  async execute({ text }) {
    // Reuse existing client-side application logic and update the UI visibly.
...
5. **Response**: The page's callback returns a result string back to the agent, which processes it alongside the visible page state to continue collaborating with the user.

### DESIGN.md and WebMCP

`DESIGN.md` is an authoring artifact for describing a visual identity to coding agents. In this repository, the [`webmcp-design-md`](skills/webmcp-design-md/SKILL.md) skill connects that artifact to WebMCP's page-local tool model; it does not add a new WebMCP API.

The processing model follows the conventional document pipeline:

```text
source text or live page
  → lexer/tokenizer: characters → tokens
  → parser: tokens → abstract syntax tree (AST)
  → semantic resolution: AST → design tokens and component states
  → renderer/generator: semantic model → DESIGN.md or an export
```

The YAML front matter carries exact machine-readable tokens. The Markdown body carries the human-readable rationale and usage rules. The page's tools may expose analysis, validation, comparison, rendering, or preview as ordinary WebMCP tools, each with a name, description, input schema, execution callback, lifecycle, and visible UI state. The agent discovers the tools through `getTools()`, invokes the selected tool with `executeTool()`, and verifies the returned result against the rendered page.

The design contract includes the complete UI↔UX connection: a token maps to a component state, that state exposes an affordance for an intent, and the resulting feedback or recovery remains understandable to a person. Loading, disabled, success, empty, error, cancellation, focus, keyboard, status, contrast, confirmation, and human takeover are part of the journey. A successful tool result does not replace visible state or user control.

### Declarative API

For forms and standard HTML inputs, a declarative counterpart to the imperative API allows the browser to automatically synthesize tool definitions from `<form>` elements. Its current proposal status is documented separately in the [Declarative API Explainer](../declarative-api-explainer.md).
...
await document.modelContext.registerTool({
  ...
  execute() { return JSON.stringify({ office: "Building 4" }); }
...
- **Service workers integration**: Extending WebMCP to background Service Workers to allow agents to discover and invoke tools on sites the user doesn't currently have open. This is detailed in the supplementary [Service Workers Explainer](service-workers.md), which proposes background discovery mechanisms, session identification, and JIT worker installation.
